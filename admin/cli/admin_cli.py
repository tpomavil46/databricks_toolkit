...
Examples:
  ...
  python admin/cli/admin_cli.py create-cluster --name "test-cluster" --workers 2
    ...
    parser.add_argument(
        "--node-type", default="i3.xlarge", help="Node type for cluster"
    )
    parser.add_argument(
        "--spark-version", default="13.3.x-scala2.12", help="Spark version"
    )
    parser.add_argument(
        "--autotermination", type=int, default=60, help="Auto-termination minutes"
    )
...
def execute_grant_entitlement(admin_client: AdminClient, args):
    ...
    if not args.username or not args.entitlement_type or not args.entitlement_value:
        print(
            "❌ Error: --username, --entitlement-type, and --entitlement-value are required"
        )
        return
...
def execute_add_user_to_group(admin_client: AdminClient, args):
    ...
    result = admin_client.privileges.add_user_to_group(
        user_name=args.username, group_name=args.group_name
    )
