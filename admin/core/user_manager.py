...
class UserManager:
    ...
    @log_function_call
    def create_user(
        self, user_name: str, display_name: str, email: str, groups: List[str] = None
    ) -> Dict[str, Any]:
        """
        Create a new user.
        ...
        try:
            ...
            user = self.client.users.create(
                user_name=user_name, display_name=display_name, email=email
            )
            ...
            if groups:
                for group_name in groups:
                    try:
                        ...
                        print(
                            f"⚠️  Warning: Failed to add user to group {group_name}: {str(e)}"
                        )
    ...
    def update_user(self, user_name: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # Update user
            updated_user = self.client.users.update(id=user.id, **kwargs)
    ...
    def get_user_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            users_with_email = len(
                [u for u in users if hasattr(u, "email") and u.email]
            )
