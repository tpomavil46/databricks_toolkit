...
class WorkspaceManager:
    ...
    def get_workspace_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            notebooks = len([obj for obj in workspace_objects if getattr(obj, 'object_type', None) == 'NOTEBOOK'])
            directories = len([obj for obj in workspace_objects if getattr(obj, 'object_type', None) == 'DIRECTORY'])
            libraries = len([obj for obj in workspace_objects if getattr(obj, 'object_type', None) == 'LIBRARY'])
            
            # Calculate total objects
            total_objects = len(workspace_objects)
            
            # Get workspace path depth distribution
            path_depths = [len(getattr(obj, 'path', '').split('/')) for obj in workspace_objects]
    ...
    def get_workspace_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            workspace_info = {
                ...
                'current_user': {
                    'user_name': getattr(current_user, 'user_name', None) if current_user else None,
                    'display_name': getattr(current_user, 'display_name', None) if current_user else None,
                    'active': getattr(current_user, 'active', True) if current_user else False
                },
                'capabilities': {
    ...
    def print_workspace_summary(self) -> None:
        ...
        print(f"\n🔧 Capabilities:")
        print(f"   Workspace Access: {'✅' if capabilities.get('can_access_workspace') else '❌'}")
        print(f"   List Objects: {'✅' if capabilities.get('can_list_objects') else '❌'}")
        print(f"   Read Objects: {'✅' if capabilities.get('can_read_objects') else '❌'}")
    ...
    def run_workspace_audit(self) -> Dict[str, Any]:
        ...
        try:
            ...
            audit_results['checks']['workspace_access'] = {
                ...
                'recommendation': 'Check workspace permissions' if not workspace_accessible else 'Workspace access confirmed'
            }
            ...
            audit_results['checks']['object_listing'] = {
                ...
                'recommendation': 'Check workspace permissions' if not can_list_objects else 'Object listing confirmed'
            }
            ...
            audit_results['checks']['object_reading'] = {
                ...
                'recommendation': 'Check workspace permissions' if not can_read_objects else 'Object reading confirmed'
            }
            ...
            if 'error' not in workspace_stats:
                ...
                audit_results['checks']['workspace_organization'] = {
                    ...
                    'recommendation': 'Consider organizing workspace structure' if not organization_good else 'Workspace organization looks good'
                }
            else:
                audit_results['checks']['workspace_organization'] = {
                    'status': 'FAIL',
