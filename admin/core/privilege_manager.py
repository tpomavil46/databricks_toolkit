...
class PrivilegeManager:
    ...
    @log_function_call
    def grant_entitlement(self, user_name: str, entitlement_type: str, entitlement_value: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                'success': False,
                'error': f"Failed to grant entitlement: {str(e)}"
            }
    
    @log_function_call
    def revoke_entitlement(self, user_name: str, entitlement_type: str, entitlement_value: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for entitlement in current_entitlements['entitlements']:
                if not (entitlement['type'] == entitlement_type and entitlement['value'] == entitlement_value):
        ...
        try:
            if user_name:
                ...
                return {
                    ...
                    'groups': groups,
                    'summary': {
                        'total_entitlements': entitlements.get('total_entitlements', 0) if entitlements['success'] else 0,
                        'total_groups': groups.get('total_groups', 0) if groups['success'] else 0,
                        'has_admin_access': self._check_admin_access(entitlements),
                        'has_cluster_access': self._check_cluster_access(entitlements),
                        'has_workspace_access': self._check_workspace_access(entitlements)
                    }
                }
            else:
                ...
                for user in users:
                    ...
                    user_summaries.append({
                        ...
                        'entitlements': entitlements.get('total_entitlements', 0) if entitlements['success'] else 0,
                        'groups': groups.get('total_groups', 0) if groups['success'] else 0,
                        'has_admin_access': self._check_admin_access(entitlements),
                        'has_cluster_access': self._check_cluster_access(entitlements),
                        'has_workspace_access': self._check_workspace_access(entitlements)
                    })
                ...
                return {
                    ...
                    'summary': {
                        'users_with_admin': len([u for u in user_summaries if u['has_admin_access']]),
                        'users_with_cluster_access': len([u for u in user_summaries if u['has_cluster_access']]),
                        'users_with_workspace_access': len([u for u in user_summaries if u['has_workspace_access']]),
                        'average_entitlements': sum(u['entitlements'] for u in user_summaries) / len(user_summaries) if user_summaries else 0,
                        'average_groups': sum(u['groups'] for u in user_summaries) / len(user_summaries) if user_summaries else 0
                    }
                }
    ...
    def _check_admin_access(self, entitlements: Dict[str, Any]) -> bool:
        ...
        admin_entitlements = ['allow-cluster-create', 'allow-instance-pool-create', 'workspace-access']
    ...
    def _check_workspace_access(self, entitlements: Dict[str, Any]) -> bool:
        ...
        return any(workspace in user_entitlements for workspace in workspace_entitlements)
    ...
    def print_privilege_summary(self, user_name: str = None) -> None:
        ...
        if user_name:
            ...
            print(f"🔧 Admin Access: {'✅' if summary['summary']['has_admin_access'] else '❌'}")
            print(f"🖥️  Cluster Access: {'✅' if summary['summary']['has_cluster_access'] else '❌'}")
            print(f"📁 Workspace Access: {'✅' if summary['summary']['has_workspace_access'] else '❌'}")
        else:
            # All users summary
            print(f"📊 Total Users: {summary['total_users']}")
            print(f"🔧 Users with Admin Access: {summary['summary']['users_with_admin']}")
            print(f"🖥️  Users with Cluster Access: {summary['summary']['users_with_cluster_access']}")
            print(f"📁 Users with Workspace Access: {summary['summary']['users_with_workspace_access']}")
            print(f"📈 Average Entitlements: {summary['summary']['average_entitlements']:.1f}")
            ...
            for user in summary['user_summaries']:
                ...
                print(f"      📊 Entitlements: {user['entitlements']}, Groups: {user['groups']}")
        ...
        try:
            ...
            if 'success' not in summary or summary['success']:
                ...
                audit_results['checks']['admin_distribution'] = {
                    ...
                    'recommendation': 'Consider reducing admin privileges' if admin_ratio > 0.2 else 'Admin distribution looks good'
                }
            else:
                audit_results['checks']['admin_distribution'] = {
                    'status': 'FAIL',
            ...
            if 'success' not in summary or summary['success']:
                ...
                audit_results['checks']['entitlement_coverage'] = {
                    ...
                    'recommendation': 'Consider granting more entitlements' if avg_entitlements < 2 else 'Entitlement coverage looks good'
                }
            else:
                audit_results['checks']['entitlement_coverage'] = {
                    'status': 'FAIL',
            ...
            if 'success' not in summary or summary['success']:
                ...
                audit_results['checks']['group_membership'] = {
                    ...
                    'recommendation': 'Consider adding users to groups' if avg_groups < 1 else 'Group membership looks good'
                }
            else:
                audit_results['checks']['group_membership'] = {
                    'status': 'FAIL',
            ...
            if 'success' not in summary or summary['success']:
                ...
                audit_results['checks']['privilege_consistency'] = {
                    ...
                    'recommendation': 'Ensure consistent privilege distribution' if not consistency_good else 'Privilege consistency looks good'
                }
            else:
                audit_results['checks']['privilege_consistency'] = {
                    'status': 'FAIL',
            ...
            audit_results['overall'] = {
                ...
                'status': 'PASS' if audit_score >= 80 else 'WARN' if audit_score >= 60 else 'FAIL'
            }
