...
class SecurityManager:
    ...
    def print_security_summary(self) -> None:
        ...
        print(f"\n🛡️  Security Features:")
        print(f"   Unity Catalog: {'✅' if security_features.get('unity_catalog_enabled') else '❌'}")
        print(f"   Workspace Isolation: {'✅' if security_features.get('workspace_isolation') else '❌'}")
        print(f"   Audit Logging: {'✅' if security_features.get('audit_logging') else '❌'}")
    ...
    def run_security_audit(self) -> Dict[str, Any]:
        ...
        try:
            ...
            audit_results['checks']['admin_access'] = {
                ...
                'recommendation': 'Ensure admin access for security operations' if not is_admin else 'Admin access confirmed'
            }
            
            # Check 2: User authentication
            current_user = self.client.current_user.me()
            user_active = getattr(current_user, 'active', True) if current_user else False
            audit_results['checks']['user_authentication'] = {
                ...
                'recommendation': 'Ensure user account is active' if not user_active else 'User authentication confirmed'
            }
            ...
            audit_results['checks']['unity_catalog'] = {
                ...
                'recommendation': 'Consider enabling Unity Catalog for enhanced governance' if not unity_enabled else 'Unity Catalog enabled'
            }
