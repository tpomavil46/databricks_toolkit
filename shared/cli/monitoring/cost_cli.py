...
@log_function_call
def get_cluster_cost_analysis(
    admin_client: AdminClient, days: int = 30
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if len(running_clusters) > 3:
            cost_analysis["recommendations"].append(
                "Consider stopping unused clusters to reduce costs"
            )

        if len(stopped_clusters) > 5:
            cost_analysis["recommendations"].append(
                "Consider deleting terminated clusters to clean up workspace"
            )
...
def get_storage_cost_analysis(admin_client: AdminClient) -> Dict[str, Any]:
    ...
    try:
        ...
        estimated_storage_cost = _estimate_storage_cost(
            total_objects, notebooks, libraries
        )
        ...
        if total_objects > 1000:
            storage_analysis["recommendations"].append(
                "Consider cleaning up unused notebooks and files"
            )
...
def get_usage_analytics(admin_client: AdminClient, days: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        if usage_analytics["user_activity"]["utilization_rate"] < 50:
            usage_analytics["recommendations"].append(
...
Examples:
  ...
  python cli/monitoring/cost_cli.py usage-analytics --days 30
    ...
    parser.add_argument(
        "command",
        choices=["cluster-costs", "storage-costs", "usage-analytics"],
