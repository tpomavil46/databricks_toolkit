...
def setup_billing_export(
    project_id: str, dataset_id: str = "billing_export"
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            os.system(
                f"gcloud services enable billingbudgets.googleapis.com --project={project_id}"
            )
            os.system(
                f"gcloud services enable bigquery.googleapis.com --project={project_id}"
            )
            os.system(
                f"gcloud services enable cloudbilling.googleapis.com --project={project_id}"
            )
        ...
        try:
            ...
            if billing_account:
                ...
                setup_results["steps_completed"].append(
                    f"Found billing account: {billing_account.name}"
                )

                # Note: Setting up billing export requires specific permissions
                print(
                    "ℹ️ To complete billing export setup, run these commands manually:"
                )
                print(f"gcloud billing accounts list")
                print(
                    f"gcloud billing export create --billing-account={billing_account.name} --dataset={dataset_id} --project={project_id}"
                )
        ...
        try:
            ...
            if current_user:
                ...
                setup_results["steps_completed"].append(
                    f"Identified current user: {current_user}"
                )

                # Grant BigQuery Admin role
                os.system(
                    f"gcloud projects add-iam-policy-binding {project_id} --member=user:{current_user} --role=roles/bigquery.admin"
                )
                ...
                if billing_account:
                    os.system(
                        f"gcloud billing accounts add-iam-policy-binding {billing_account.name} --member=user:{current_user} --role=roles/billing.user"
                    )
                    setup_results["steps_completed"].append(
                        "Granted Billing Account User role"
                    )
    ...
    try:
        ...
        try:
            ...
            if billing_accounts:
                check_results["details"][
                    "billing"
                ] = f"✅ Billing accounts accessible: {len(list(billing_accounts))} accounts"
        ...
        try:
            ...
            check_results["details"][
                "bigquery"
            ] = f"✅ BigQuery accessible: {len(datasets)} datasets found"
...
def print_setup_instructions(project_id: str):
    ...
    print(
        "   gcloud billing export create --billing-account=BILLING_ACCOUNT_ID --dataset=billing_export --project={project_id}"
    )
    print()

    print("5. Grant necessary permissions:")
    print(
        f"   gcloud projects add-iam-policy-binding {project_id} --member=user:$(gcloud auth list --filter=status:ACTIVE --format='value(account)') --role=roles/bigquery.admin"
    )
    print()

    print("6. Test the setup:")
    print(
        f"   python shared/cli/monitoring/gcp_cost_cli.py --project {project_id} --costs"
    )
    print()
...
Examples:
  ...
  python setup_gcp_billing.py --project myproject --instructions
    ...
    parser.add_argument("--setup", action="store_true", help="Set up billing export")

    parser.add_argument(
        "--check", action="store_true", help="Check billing setup status"
    )
