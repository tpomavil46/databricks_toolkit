...
def get_gcp_billing_data(project_id: str, days: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        if not billing_account_id:
            billing_data["error"] = (
                "No billing account found or billing export not enabled"
            )
            return billing_data

        # Query Standard usage cost data
        standard_costs = _get_standard_usage_costs(
            bq_client, project_id, billing_account_id, days
        )

        # Query Detailed usage cost data (if available)
        detailed_costs = _get_detailed_usage_costs(
            bq_client, project_id, billing_account_id, days
        )
        ...
        if billing_data["total_cost"] > 100:
            billing_data["recommendations"].append(
...
@log_function_call
def _get_standard_usage_costs(
    bq_client, project_id: str, billing_account_id: str, days: int
    ...
    try:
        ...
        for row in results:
            ...
            if service_name in costs:
                ...
                costs[service_name]["usage"] += (
                    float(row.total_usage) if row.total_usage else 0
                )
            else:
                costs[service_name] = {
                    "cost": float(row.total_cost),
...
def get_cost_trends(project_id: str, days: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        # Query daily costs
        daily_costs = _get_daily_cost_trends(
            bq_client, project_id, billing_account_id, days
        )

        trends_data["daily_costs"] = daily_costs
        trends_data["total_cost"] = sum(day["cost"] for day in daily_costs)
        trends_data["average_daily_cost"] = (
            trends_data["total_cost"] / len(daily_costs) if daily_costs else 0
        )
...
def main():
    ...
    parser.add_argument(
        "--trends",
        action="store_true",
        help="Show cost trends instead of current costs",
    )
    parser.add_argument(
