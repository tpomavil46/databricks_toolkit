...
def run_system_health_check(admin_client: AdminClient) -> Dict[str, Any]:
    ...
    try:
        ...
        workspace_health = {
            "status": (
                "PASS"
                if workspace_info.get("capabilities", {}).get("can_access_workspace")
                else "FAIL"
            ),
        ...
        running_clusters = [
            c for c in clusters if getattr(c, "state", "UNKNOWN") == "RUNNING"
        ]
        cluster_health = {
        ...
        if len(running_clusters) == 0:
            health_results["recommendations"].append(
                "Consider starting at least one cluster for data processing"
            )

        if len(active_users) == 0:
            health_results["recommendations"].append(
                "No active users found - check user management"
            )

        if security_audit.get("overall", {}).get("status") != "PASS":
            health_results["recommendations"].append(
                "Security audit failed - review security configuration"
            )
...
def run_cluster_health_check(
    ...
    try:
        ...
        passed_checks = sum(
            1
            for check in cluster_results["checks"].values()
            if check["status"] == "PASS"
        )
        ...
        if (
            cluster_id
            and cluster_results["checks"].get("cluster_state", {}).get("status")
            == "FAIL"
        ):
            cluster_results["recommendations"].append(
                f"Cluster {cluster_id} is not running - consider starting it"
            )

        if not cluster_id and len(running_clusters) == 0:
            cluster_results["recommendations"].append(
                "No running clusters - consider starting at least one cluster"
            )
...
@log_function_call
def run_user_health_check(
    admin_client: AdminClient, username: str = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        # Determine overall status
        passed_checks = sum(
            1 for check in user_results["checks"].values() if check["status"] == "PASS"
        )
        ...
        if (
            username
            and user_results["checks"].get("user_status", {}).get("status") == "FAIL"
        ):
            user_results["recommendations"].append(
                f"User {username} is inactive - consider activating"
            )

        if not username and len(active_users) == 0:
            user_results["recommendations"].append(
                "No active users - check user management"
            )
...
Examples:
  ...
  python cli/monitoring/health_cli.py user-health --username john.doe
    ...
    parser.add_argument(
        "command",
        choices=["system-health", "cluster-health", "user-health"],
        help="Health check command to execute",
    )

    parser.add_argument("--cluster-id", help="Specific cluster ID to check")

    parser.add_argument("--username", help="Specific username to check")
