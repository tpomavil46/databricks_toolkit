...
class DynamicDashboard:
    ...
    def get_table_details_from_databricks(self, catalog: str, schema: str, table: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                if len(table_parts) == 3:
                    ...
                    metrics['column_count'] = str(len(df.columns)) if not df.empty else "0"
                ...
                metrics['health_status'] = random.choice(['healthy', 'warning', 'stale'])
    ...
    def render_pipeline_summary(self, pipeline_components: List[Dict[str, Any]]):
        ...
        with col4:
            ...
            layer_text = ", ".join([f"{count} {layer}" for layer, count in layer_counts.items()])
            st.metric("🏗️ Layer Distribution", layer_text[:20] + "..." if len(layer_text) > 20 else layer_text)
    ...
    def render_table_details(self, component: Dict[str, Any], show_collapse: bool = True):
        ...
        if not pipeline_components:
            ...
            for i, component in enumerate(pipeline_components):
                ...
                if metrics:
                    ...
                    if metrics.get('health_status'):
                        status_emoji = "🟢" if metrics['health_status'] == 'healthy' else "🔴"
                        annotation_text += f" | {status_emoji} {metrics['health_status']}"
                else:
    ...
    def render_dashboard_view(self):
        ...
        with col2:
            if st.button("✏️ Edit in Builder"):
                ...
                if dashboard_config:
                    st.session_state['pipeline_components'] = dashboard_config.get('pipeline_components', [])
                    st.session_state['dashboard_title'] = dashboard_config.get('title', '')
                    st.session_state['dashboard_description'] = dashboard_config.get('description', '')
                    st.session_state['saved_charts'] = dashboard_config.get('charts', [])
                    st.session_state['layout_config'] = dashboard_config.get('layout', {'type': 'vertical'})
                st.rerun()
        ...
        if pipeline_components:
            ...
            st.plotly_chart(arch_fig, use_container_width=True, key="dashboard_view_arch_diagram")
            ...
            tab_names = [f"{comp.get('layer_type', 'Custom')}: {comp.get('full_name', comp.get('table_name', 'Unknown'))}" 
    ...
    def render_charts_with_layout(self, charts: List[Dict], layout_config: Dict):
        ...
        visible_charts = [chart for chart in charts if chart.get('layout', {}).get('visible', True)]
    ...
    def render_chart_builder(self):
        ...
        if st.session_state.get('pipeline_components'):
            ...
            with col1:
                if st.button("💾 Save Dashboard") and dashboard_name:
                    ...
                    layout_config = st.session_state.get('layout_config', {'type': 'vertical'})
                ...
                if len(st.session_state['saved_charts']) > 1:
                    ...
                    with col1:
                        move_from = st.selectbox(
                            "Move chart from:",
                            options=[f"{i+1}. {chart['title'] or chart['chart_type']}" for i, chart in enumerate(st.session_state['saved_charts'])],
                            key="move_from"
                        )
                    with col2:
                        move_to = st.selectbox(
                            "To position:",
                            options=[i+1 for i in range(len(st.session_state['saved_charts']))],
                            key="move_to"
                        )
                ...
                for i, chart_config in enumerate(st.session_state['saved_charts']):
                    with st.expander(f"📊 {chart_config['title'] or chart_config['chart_type']}", expanded=False):
                        ...
                        with col2:
                            ...
                            # Chart width control
                            current_width = chart_config.get('layout', {}).get('width', 'Full')
                            new_width = st.selectbox(
                                "Width",
                                ["Full", "Half", "Third", "Quarter"],
                                index=["Full", "Half", "Third", "Quarter"].index(current_width),
                            ...
                            current_position = chart_config.get('layout', {}).get('position', 'Auto')
                            new_position = st.selectbox(
                                "Position",
                                ["Auto", "Left", "Center", "Right"],
                                index=["Auto", "Left", "Center", "Right"].index(current_position),
                            ...
                            # Visibility toggle
                            current_visible = chart_config.get('layout', {}).get('visible', True)
        ...
        if st.session_state.get('pipeline_components'):
            ...
            with col2:
                if st.button("💾 Quick Save Dashboard", key="quick_save_button") and quick_dashboard_name:
                    # Save dashboard configuration (without DataFrame objects)
                    pipeline_components_clean = []
                    for comp in st.session_state['pipeline_components']:
                        clean_comp = {
                    ...
                    dashboard_file = dashboards_dir / f"{quick_dashboard_name.replace(' ', '_').lower()}.json"
                    with open(dashboard_file, 'w') as f:
                        json.dump(dashboard_config, f, indent=2)
    ...
    def render_pipeline_builder(self):
        ...
        if pipeline_components:
            ...
            st.info("No pipeline components found. Add some components in the sidebar to see table details.")
        
        # Show data for each component if loaded
        if pipeline_components and any(comp.get('data') is not None for comp in pipeline_components):
    ...
    def render_gcp_dashboard(self):
        ...
        **Data Sources**:
        ...
        if not self.gcp.is_configured():
            ...
            st.info("💡 Set up GCP authentication with: `gcloud auth application-default login`")
            return
        ...
        with col3:
            status = "✅ Configured" if self.gcp.is_configured() else "❌ Not Configured"
        ...
        if real_time_costs:
            ...
            with col3:
                ...
                monthly_projection = total_real_time * (days_in_month / current_date.day) if current_date.day > 0 else 0
                ...
                for service, cost in real_time_costs.items():
                    if cost > 0:  # Only show services with actual costs
                        cost_data.append({
                            ...
                            'Cost': cost,
                            'Percentage': (cost / total_real_time * 100) if total_real_time > 0 else 0
                        })
            ...
            for service, cost in cloud_billing_costs.items():
                if cost > 0:  # Only show services with costs
                    cloud_billing_data.append({
                        'Service': service.replace('_', ' ').title(),
                        'Cost': cost,
                        'Percentage': (cost / total_cloud_billing_cost * 100) if total_cloud_billing_cost > 0 else 0
                    })
        ...
        if detailed_costs:
            ...
            # Calculate totals
            total_usage_cost = sum(cost['usage_cost'] for cost in detailed_costs.values())
            total_savings_programs = sum(cost['savings_programs'] for cost in detailed_costs.values())
            total_other_savings = sum(cost['other_savings'] for cost in detailed_costs.values())
        ...
        if detailed_costs:
            ...
            if total_savings_programs > 0:
                alerts.append(f"💰 **Savings Active**: ${total_savings_programs:.2f} in savings programs")
            
            if total_other_savings > 0:
                alerts.append(f"💳 **Credits Applied**: ${total_other_savings:.2f} in other savings")
        ...
        available_services = {name: df for name, df in all_services.items() if not df.empty}
        
        if available_services:
            # Create tabs for each service with data
            service_tabs = st.tabs([service.replace('_', ' ').title() for service in available_services.keys()])
            
            for i, (service_name, df) in enumerate(available_services.items()):
                with service_tabs[i]:
                    if not df.empty:
                        # Add time period indicator for this service
                        st.info(f"📅 **{service_name.replace('_', ' ').title()} Data Period**: {current_month}")
                        ...
                        with col4:
                            ...
                            if service_name == 'bigquery':
                                ...
                                st.metric("Buckets", f"{df.get('bucket_count', 0).sum()}")
                        ...
                        elif 'is_real_data' in df.columns and df['is_real_data'].iloc[0]:
                        ...
                        # Cost trend chart
                        if len(df) > 1:
                            fig = px.line(df, x='date', y='cost', title=f'{service_name.replace("_", " ").title()} Cost Trend')
                            st.plotly_chart(fig, use_container_width=True)
    ...
    def render_dashboard_builder(self):
        ...
        with st.sidebar:
            ...
            with col2:
                if st.button("📊 Business Analytics", help="Navigate to Business Analytics Dashboard"):
