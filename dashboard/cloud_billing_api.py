...
class CloudBillingAPI:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            print("❌ Cloud Billing API not available. Install with: pip install google-cloud-billing")
    ...
    def is_configured(self) -> bool:
        ...
        return (
            ...
            self.billing_account_id is not None and
    ...
    def _query_billing_api_costs(self, start_date: datetime, end_date: datetime) -> Dict[str, float]:
        """
        ...
        try:
            ...
            try:
                ...
                costs = self._get_costs_from_billing_account(billing_account_name, start_date, end_date)
    ...
    def _get_costs_from_billing_account(self, billing_account_name: str, start_date: datetime, end_date: datetime) -> Dict[str, float]:
        """
        ...
        """
        try:
            print("🔍 Attempting to get costs from billing account using Cloud Billing API...")
            ...
            for service in services[:5]:  # Limit to first 5 services for testing
                try:
                    service_costs = self._get_service_costs_from_api(service, billing_account_name)
                    if service_costs > 0:
    ...
    def _get_available_services(self) -> List[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                service_names = [service.get('serviceId') for service in services if service.get('serviceId')]
                print(f"✅ Retrieved {len(service_names)} services from Cloud Billing API REST call")
                return service_names
            else:
                print(f"❌ Cloud Billing API call failed: {response.status_code} - {response.text}")
                return []
    ...
    def _get_service_costs_from_api(self, service_id: str, billing_account_name: str) -> float:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                skus = data.get('skus', [])
                print(f"✅ Retrieved {len(skus)} SKUs for {service_id} via Cloud Billing API REST call")
                
                # For now, return a placeholder cost
                # The actual cost calculation would require usage data from other APIs
                return 0.0
            else:
                print(f"❌ Cloud Billing API SKU call failed: {response.status_code} - {response.text}")
                return 0.0
    ...
    def get_service_costs(self, service_name: str, days: int = 30) -> pd.DataFrame:
        ...
        try:
            ...
            service_costs = self._get_service_costs_from_api(service_name, f"billingAccounts/{self.billing_account_id}")
    ...
    def _get_service_costs_from_bigquery(self, service_name: str, start_date: datetime, end_date: datetime) -> pd.DataFrame:
        """
        ...
        try:
            ...
            service_description = service_mapping.get(service_name, service_name.replace('_', ' ').title())
            ...
            for table_name in tables_to_try:
                try:
                    ...
                    if data:
                        print(f"✅ Found {len(data)} records for {service_name} from {table_name}")
                        return pd.DataFrame(data)
    ...
    def get_historical_costs(self, days: int = 30) -> Dict[str, float]:
        ...
        try:
            ...
            print(f"🔍 Getting historical costs for the last {days} days using Cloud Billing API...")
            ...
            if costs:
                total_cost = sum(costs.values())
                print(f"✅ Found ${total_cost:.2f} in historical costs via Cloud Billing API")
                return costs
            else:
