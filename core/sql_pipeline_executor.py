...
class SQLPipelineExecutor:
    ...
    def execute_sql(self, config: SQLPipelineConfig) -> Optional[DataFrame]:
        ...
        try:
            ...
            is_ddl = any(keyword in sql_upper for keyword in ['CREATE', 'DROP', 'ALTER', 'INSERT', 'UPDATE', 'DELETE'])
            ...
            if config.create_temp_view:
                ...
                if not config.table_name:
                    raise ValueError("table_name is required when write_to_table is True")
                
                result.write.format(config.write_format).mode(config.write_mode).saveAsTable(config.table_name)
    ...
    @log_function_call
    def execute_pipeline(self, pipeline_configs: List[SQLPipelineConfig]) -> Dict[str, Any]:
        """
...
class SQLPipelineBuilder:
    ...
    def write_to_table(self, table_name: str, format: str = "delta", mode: str = "overwrite") -> 'SQLPipelineBuilder':
    ...
    def with_schema_validation(self, expected_schema: StructType) -> 'SQLPipelineBuilder':
