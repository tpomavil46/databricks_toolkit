...
class DataTransformation:
    ...
    def add_metadata_columns(
        self, df: DataFrame, source_path: str, layer: str
    ) -> DataFrame:
        """
    ...
    def clean_string_columns(
        self, df: DataFrame, columns: List[str] = None
    ) -> DataFrame:
        """
        ...
        for column in string_columns:
            if column in df.columns:
                result_df = result_df.withColumn(column, trim(upper(col(column))))

        return result_df

    @log_function_call
    def handle_null_values(
        self, df: DataFrame, null_strategy: Dict[str, str] = None
    ) -> DataFrame:
        """
                ...
                if strategy == "drop":
                    ...
                    if isinstance(df.schema[column].dataType, StringType):
                        result_df = result_df.withColumn(
                            column,
                            when(col(column).isNull(), lit("UNKNOWN")).otherwise(
                                col(column)
                            ),
                        )
                    else:
                    ...
                    result_df = result_df.withColumn(
                        column,
                        when(col(column).isNull(), lit(mean_value)).otherwise(
                            col(column)
                        ),
                    )
                    ...
                    median_value = (
                        df.select(col(column))
                        .filter(col(column).isNotNull())
                        .agg({column: "percentile"})
                        .collect()[0][0]
                    )
                    result_df = result_df.withColumn(
                        column,
                        when(col(column).isNull(), lit(median_value)).otherwise(
                            col(column)
                        ),
                    )
    ...
    def convert_data_types(
        self, df: DataFrame, type_mapping: Dict[str, str] = None
    ) -> DataFrame:
        """
        ...
        for column, target_type in type_mapping.items():
            if column in df.columns:
                if target_type == "string":
                    ...
                    result_df = result_df.withColumn(
                        column, col(column).cast(TimestampType())
                    )
                elif target_type == "date":
                    result_df = result_df.withColumn(
                        column, col(column).cast(DateType())
                    )
    ...
    def remove_duplicates(
        self, df: DataFrame, key_columns: List[str] = None, keep: str = "first"
    ) -> DataFrame:
        """
        ...
        if keep == "first":
            ...
            return df.orderBy(*[col(c).desc() for c in key_columns]).dropDuplicates(
                key_columns
            )

    @log_function_call
    def filter_data(
        self, df: DataFrame, filter_conditions: Dict[str, Any] = None
    ) -> DataFrame:
        """
    ...
    def apply_custom_transformation(
        self, df: DataFrame, transformation_func: Callable
    ) -> DataFrame:
        """
    ...
    def create_bronze_table(
        self, df: DataFrame, table_name: str, source_path: str
    ) -> DataFrame:
        """
    ...
    def create_silver_table(
        self, df: DataFrame, table_name: str, transformations: Dict[str, Any] = None
    ) -> DataFrame:
        """
        ...
        if transformations:
            ...
            if "null_strategy" in transformations:
                silver_df = self.handle_null_values(
                    silver_df, transformations["null_strategy"]
                )

            # Apply data type conversions
            if "type_mapping" in transformations:
                silver_df = self.convert_data_types(
                    silver_df, transformations["type_mapping"]
                )

            # Apply filters
            if "filter_conditions" in transformations:
                silver_df = self.filter_data(
                    silver_df, transformations["filter_conditions"]
                )

            # Remove duplicates
            if "deduplication_keys" in transformations:
                silver_df = self.remove_duplicates(
                    silver_df, transformations["deduplication_keys"]
                )

            # Rename columns
            if "column_mapping" in transformations:
                silver_df = self.rename_columns(
                    silver_df, transformations["column_mapping"]
                )

            # Select specific columns
            if "select_columns" in transformations:
                silver_df = self.select_columns(
                    silver_df, transformations["select_columns"]
                )

        # Add metadata
        silver_df = self.add_metadata_columns(
            silver_df, "silver_transformation", "silver"
        )
    ...
    def create_gold_table(
        self, df: DataFrame, table_name: str, aggregations: Dict[str, Any] = None
    ) -> DataFrame:
        """
            ...
            if "group_by" in aggregations and "aggregations" in aggregations:
                ...
                for col_name, agg_func in agg_cols.items():
                    if col_name in df.columns:
                        agg_exprs.append(
                            agg_func(col(col_name)).alias(
                                f"{col_name}_{agg_func.__name__}"
                            )
                        )
