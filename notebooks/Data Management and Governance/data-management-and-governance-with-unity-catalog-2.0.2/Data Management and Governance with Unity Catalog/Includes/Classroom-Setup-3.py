...
create_user_schema_in_hive = spark.sql(
    f"CREATE SCHEMA IF NOT EXISTS hive_metastore.{DA.user_hive_schema}"
)

## Create the movies table in the hive_metastore catalog, user's schema name
create_movies_table_in_hive = spark.sql(
    f"""
...
print(
    f"Created a schema in the hive_metastore named {DA.user_hive_schema} for the user and populated it with the movies table."
)
...
DA.display_config_values(
    [
        ...
        (
            "Your Schema in the hive_metastore catalog",
            f"hive_metastore.{DA.user_hive_schema}",
        ),
    ]
)
