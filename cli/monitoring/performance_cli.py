...
@log_function_call
def get_cluster_performance_metrics(admin_client: AdminClient, cluster_id: str = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if cluster_id:
            ...
            if state != 'RUNNING':
                performance_metrics['recommendations'].append(f'Cluster {cluster_id} is not running - consider starting it')
            
            if config.get('num_workers', 0) == 0:
                performance_metrics['recommendations'].append('Single-node cluster detected - consider adding workers for better performance')
            ...
            total_workers = sum(c.get('cluster_config', {}).get('num_workers', 0) for c in running_clusters)
            avg_workers = total_workers / len(running_clusters) if running_clusters else 0
            ...
            if len(running_clusters) == 0:
                performance_metrics['recommendations'].append('No running clusters - start clusters for data processing')
            
            if avg_workers < 2 and len(running_clusters) > 0:
                performance_metrics['recommendations'].append('Low worker count - consider adding workers for better performance')
            
            if len(stopped_clusters) > len(running_clusters):
                performance_metrics['recommendations'].append('More stopped than running clusters - consider cleanup')
...
def get_system_performance_metrics(admin_client: AdminClient) -> Dict[str, Any]:
    ...
    try:
        ...
        system_metrics['metrics'] = {
            ...
            'total_clusters': len(clusters),
            'running_clusters': len(running_clusters),
            'cluster_utilization': (len(running_clusters) / len(clusters)) * 100 if clusters else 0
        }
        
        # Generate system recommendations
        if system_metrics['metrics']['user_utilization'] < 50:
            system_metrics['recommendations'].append('Low user utilization - consider user training or cleanup')
        
        if system_metrics['metrics']['cluster_utilization'] < 30:
            system_metrics['recommendations'].append('Low cluster utilization - consider stopping unused clusters')
        
        if system_metrics['metrics']['workspace_objects'] > 1000:
            system_metrics['recommendations'].append('High object count - consider workspace cleanup')
        
        if system_metrics['metrics']['avg_path_depth'] > 5:
            system_metrics['recommendations'].append('Deep directory structure - consider reorganizing workspace')
...
def get_job_performance_metrics(admin_client: AdminClient) -> Dict[str, Any]:
    ...
    try:
        ...
        job_metrics['recommendations'].append('Job monitoring requires integration with Databricks Jobs API')
        job_metrics['recommendations'].append('Consider implementing job performance tracking')
...
Examples:
  ...
  python cli/monitoring/performance_cli.py job-performance
    ...
    parser.add_argument(
        "--cluster-id",
        help="Specific cluster ID to analyze"
    )
    ...
    if args.command == "cluster-performance":
        ...
        print("=" * 50)
        performance_results = get_cluster_performance_metrics(admin_client, args.cluster_id)
