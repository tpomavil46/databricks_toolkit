...
class DatasetAnalyzer:
    ...
    def analyze_dataset(self, dataset_path: str, dataset_name: str = None, max_rows: int = 10000):
        """
    ...
    def _load_dataset(self, dataset_path: str):
        ...
        try:
            ...
            for fmt in formats_to_try:
                try:
                    if fmt == "csv":
                        df = self.spark.read.format(fmt).option("header", "true").load(dataset_path + "*")
                    else:
    ...
    def _show_data_quality(self, df, max_rows: int):
        ...
        for field in df.schema.fields:
            ...
            quality_metrics.append({
                ...
                'empty_strings': empty_string_count if isinstance(field.dataType, StringType) else 'N/A'
            })
    ...
    def _show_statistical_summary(self, df):
        ...
        if numeric_columns:
            ...
            for col_name in numeric_columns:
                print(f"\n{col_name}:")
                stats = df.select(col_name).summary("count", "min", "25%", "50%", "75%", "max")
    ...
    def _show_column_analysis(self, df, max_rows: int):
        ...
        for field in df.schema.fields:
            ...
            if isinstance(field.dataType, StringType):
                print("   Top 5 values:")
                top_values = sample_df.groupBy(col_name).count().orderBy(desc("count")).limit(5)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze datasets with comprehensive metrics.")
    parser.add_argument(
        "--path",
        type=str,
        required=True,
