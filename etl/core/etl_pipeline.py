...
class StandardETLPipeline:
    ...
    @log_function_call
    def run_bronze_ingestion(self, source_path: str, table_name: str = None, auto_drop: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Create bronze table
            bronze_df = self.transformer.create_bronze_table(df, table_name, source_path)
            
            # Validate bronze data
            validation_results = self.validator.validate_bronze_data(bronze_df, table_name)
    ...
    @log_function_call
    def run_silver_transformation(self, source_table: str, table_name: str = None, 
                                transformations: Dict[str, Any] = None, auto_drop: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            silver_df = self.transformer.create_silver_table(df, table_name, transformations)
            
            # Validate silver data
            validation_results = self.validator.validate_silver_data(silver_df, table_name)
            ...
            self.pipeline_results['silver'] = {
                ...
                'validation_results': validation_results,
                'transformations_applied': list(transformations.keys()) if transformations else [],
                'status': 'success'
            }
    ...
    @log_function_call
    def run_gold_aggregation(self, source_table: str, table_name: str = None,
                           aggregations: Dict[str, Any] = None, auto_drop: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.pipeline_results['gold'] = {
                ...
                'validation_results': validation_results,
                'aggregations_applied': list(aggregations.keys()) if aggregations else [],
                'status': 'success'
            }
    ...
    @log_function_call
    def run_full_pipeline(self, source_path: str, transformations: Dict[str, Any] = None,
                         aggregations: Dict[str, Any] = None, auto_drop: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            silver_results = self.run_silver_transformation(bronze_table, transformations=transformations, auto_drop=auto_drop)
            silver_table = silver_results['table_name']
            
            # Run gold aggregation
            gold_results = self.run_gold_aggregation(silver_table, aggregations=aggregations, auto_drop=auto_drop)
    ...
    def get_pipeline_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'successful_layers': len([r for r in self.pipeline_results.values() if r.get('status') == 'success']),
            'failed_layers': len([r for r in self.pipeline_results.values() if r.get('status') == 'failed']),
            'total_rows_processed': sum(r.get('row_count', 0) for r in self.pipeline_results.values()),
